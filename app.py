...
from detection import process_video as pv
...
def main():
    # Header
    st.markdown(
        '<h1 class="main-header">🚗 License Plate Detection System</h1>',
        unsafe_allow_html=True,
    )
        ...
        if uploaded_file is not None:
            ...
            if st.button("🚀 Process Video", type="primary"):
                ...
                if model is not None and reader is not None:
                    ...
                    # Process video
                    output_path, detections, stats = pv.process_video(
                        st,
                        video_path,
                        model,
                        reader,
        ...
        if hasattr(st.session_state, "output_path") and st.session_state.output_path:
            ...
            if st.session_state.detections:
                ...
                filtered_df = detections_df[
                    detections_df["vehicle_type"].isin(vehicle_filter)
                ]
                if plate_only:
                    filtered_df = filtered_df[
                        filtered_df["plate_text"] != "Not detected"
                    ]
