...
import time
import detect_vehicles_and_plates

def process_video(st, video_path, model, reader, confidence_threshold=0.5, process_every_n_frames=5):
    ...
    try:
        while True:
            ...
            if frame_count % process_every_n_frames == 0:
                ...
                # Process frame
                annotated_frame, detections = detect_vehicles_and_plates(
                    frame, model, reader, confidence_threshold
                )
                ...
                # Update status
                status_text.text(f"Processing: {progress*100:.1f}% | Frame {frame_count}/{total_frames} | Detections: {len(all_detections)}")
            else:
